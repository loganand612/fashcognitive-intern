  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (draggedItem.type === "section" && dropTarget.type === "section") {
      ...
      setTemplate((prev) => ({ ...prev, sections }))
        ...
        ;(newSections[dropSectionIndex].content as StandardSectionContent).questions.splice(dropQuestionIndex, 0, removedQuestion)
...
interface ReportTabProps {
  template: Template
  updateGarmentDetails: (sectionId: string, updates: Partial<GarmentDetailsContent>) => void
}

const ReportTab: React.FC<ReportTabProps> = ({ template, updateGarmentDetails }) => {
  const [reportData, setReportData] = useState<ReportData>(() => {
    const garmentDetailsSection = template.sections.find((s) => s.type === "garmentDetails")
    const initialDefects =
      garmentDetailsSection?.content.defaultDefects.map((defect) => ({
        type: defect,
        remarks: "",
        major: 0,
        minor: 0,
    ...
    const initialAqlLevel = garmentDetailsSection?.content.aqlSettings.aqlLevel || "2.5"
    const initialInspectionLevel = garmentDetailsSection?.content.aqlSettings.inspectionLevel || "II"
    const initialSamplingPlan = garmentDetailsSection?.content.aqlSettings.samplingPlan || "Single"
    const initialSeverity = garmentDetailsSection?.content.aqlSettings.severity || "Normal"
  ...
  const handleQuantityChange = useCallback(
    (color: string, size: string, field: "orderQty" | "offeredQty", value: string) => {
      setReportData((prev) => {
  ...
  const calculateRowTotal = useCallback(
    (color: string, field: "orderQty" | "offeredQty") => {
      const sizes = template.sections.find((s) => s.type === "garmentDetails")?.content.sizes || []
      let total = 0
      sizes.forEach((size) => {
    ...
    [reportData.quantities, template.sections],
  )

  const calculateColumnTotal = useCallback(
    (size: string, field: "orderQty" | "offeredQty") => {
      const colors = template.sections.find((s) => s.type === "garmentDetails")?.content.colors || []
      let total = 0
  ...
  const calculateGrandTotal = useCallback(
    ...
    [calculateRowTotal, template.sections],
  )
  ...
  const updateDefect = useCallback(
    (index: number, field: keyof Defect, value: string | number) => {
      setReportData((prev) => {
  ...
  const calculateTotalDefects = useCallback(
    (field: keyof Defect) => {
      return reportData.defects.reduce((total, defect) => {
        const value = Number.parseInt(defect[field] as string, 10)
  ...
  const addReportSize = useCallback(() => {
  ...
  }, [reportData.newSize, template.sections, updateGarmentDetails])

  const addReportColor = useCallback(() => {
  ...
  }, [reportData.newColor, template.sections, updateGarmentDetails])

  const toggleAqlEditing = useCallback(() => {
    setReportData((prev) => ({ ...prev, editingAql: !prev.editingAql }))
  }, [])

  const updateAqlResult = useCallback((field: keyof ReportData["aqlSettings"], value: string) => {
    setReportData((prev) => ({
      ...prev,
  ...
  return (
    <div className="report-preview-container">
      ...
      <p>This is a preview of how your garment inspection report will look.</p>
