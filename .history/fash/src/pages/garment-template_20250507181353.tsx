...
type SectionContent = GarmentDetailsContent | StandardSectionContent;
...
// Update the response type constants
const RESPONSE_TYPES: { [key: string]: ResponseType } = {
  Text: "Text",
  Number: "Number",
  Checkbox: "Checkbox",
  "Yes/No": "Yes/No",
...
// Update the response type display mapping
const RESPONSE_TYPE_DISPLAY: { [K in ResponseType]: string } = {
  "Site": "Site",
  "Inspection date": "Inspection date",
  "Person": "Person",
  "Inspection location": "Inspection location",
...
// Update the type guard for garment details content
const isGarmentDetailsContent = (content: SectionContent): content is GarmentDetailsContent => {
  return content && typeof content === 'object' && 'sizes' in content && 'colors' in content && 'aqlSettings' in content;
};
...
const Garment_Template: React.FC = () => {
  ...
  const [reportData, setReportData] = useState<ReportData>(() => {
    ...
    const garmentContent = garmentSection?.content as GarmentDetailsContent | undefined;

    const initialDefects = garmentContent?.defaultDefects.map((defect) => ({
      type: defect,
      remarks: "",
      critical: 0,
      major: 0,
    ...
    const initialAqlLevel = garmentContent?.aqlSettings.aqlLevel || "2.5";
    const initialInspectionLevel = garmentContent?.aqlSettings.inspectionLevel || "II";
    const initialSamplingPlan = garmentContent?.aqlSettings.samplingPlan || "Single";
    const initialSeverity = garmentContent?.aqlSettings.severity || "Normal";
  ...
  const calculateRowTotal = (color: string, field: 'orderQty' | 'offeredQty') => {
    const garmentSection = template.sections.find((s) => s.type === "garmentDetails");
    const garmentContent = garmentSection?.content as GarmentDetailsContent | undefined;
  ...
  const calculateColumnTotal = (size: string, field: 'orderQty' | 'offeredQty') => {
    const garmentSection = template.sections.find((s) => s.type === "garmentDetails");
    const garmentContent = garmentSection?.content as GarmentDetailsContent | undefined;
  ...
  const calculateGrandTotal = (field: 'orderQty' | 'offeredQty') => {
    const garmentSection = template.sections.find((s) => s.type === "garmentDetails");
    const garmentContent = garmentSection?.content as GarmentDetailsContent | undefined;
    const colors = garmentContent?.colors || []
  ...
  const calculateTotalDefects = (field: keyof Pick<typeof reportData.defects[0], 'critical' | 'major' | 'minor'>) => {
    return reportData.defects.reduce((total, defect) => {
      const value = Number.parseInt(defect[field]?.toString() || "0", 10)
  ...
  const addReportSize = () => {
    ...
    const garmentSection = template.sections.find((s) => s.type === "garmentDetails")
    if (!garmentSection) return

    const garmentContent = garmentSection.content as GarmentDetailsContent
    const updatedSizes = [...garmentContent.sizes, reportData.newSize.trim()]
    updateGarmentDetails(garmentSection.id, { sizes: updatedSizes })
  ...
  const addReportColor = () => {
    ...
    const garmentContent = garmentSection.content as GarmentDetailsContent
    const updatedColors = [...garmentContent.colors, reportData.newColor.trim()]
    updateGarmentDetails(garmentSection.id, { colors: updatedColors })
  ...
  const renderResponseTypeMenu = (questionId: string) => {
      ...
      <div className="response-type-menu">
        {responseTypes.map((type) => (
          <button
            ...
            onClick={() => handleResponseTypeChange(questionId, type)}
          >
            {renderResponseTypeIcon(type)}
            <span>{RESPONSE_TYPE_DISPLAY[type]}</span>
          </button>
        ))}
      </div>
    );
          ...
          <div className="report-section-preview">
            ...
            {template.sections.some((s) => s.type === "garmentDetails") ? (
              <div className="garment-grid-preview">
                ...
                {renderCartonInfo(garmentContent)}
  ...
  const renderCartonInfo = (garmentContent: GarmentDetailsContent) => {
    return (
      <>
  ...
  // Update the renderGarmentDetailsContent function
  const renderGarmentDetailsContent = () => {
    const garmentSection = template.sections.find((s) => s.type === "garmentDetails");
    if (!garmentSection || !isGarmentDetailsContent(garmentSection.content)) return null;
    const garmentContent = garmentSection.content;
