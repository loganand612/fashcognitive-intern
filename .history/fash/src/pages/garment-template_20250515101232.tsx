...
import { ChevronDown, ChevronUp, Edit, Plus, Calendar, User, MapPin, X, Check, ImageIcon, Trash2, Move, Clock, ArrowLeft, ArrowRight, CheckCircle, Settings, Ruler, Box, List, Shirt, FileText, Printer, CornerDownRight } from 'lucide-react'
...
const EnhancedLogicConditionSelector: React.FC<{
  ...
  useEffect(() => {
    // Set available conditions based on question type
    ...
    } else if (questionType === "Multiple choice" || questionType === "Yes/No") {
      setAvailableConditions(["is", "is not", "is one of", "is not one of"])
    } else {
      setAvailableConditions([
        "is",
        "is not",
      ...
      {isOpen && (
        <div className="conditions-dropdown">
          {availableConditions.map((condition) => (
            <div
              key={condition}
              className={`condition-option ${condition === selectedCondition ? "selected" : ""}`}
              onClick={() => {
...
const EnhancedLogicValueInput: React.FC<{
  ...
  // Helper function to handle range changes
  const updateRange = (start: string, end: string) => {
    const startNum = parseFloat(start)
    const endNum = parseFloat(end)
    if (!isNaN(startNum) && !isNaN(endNum)) {
      onChange([startNum, endNum])
    }
  }
      ...
      <div className={`enhanced-logic-range-input ${className}`}>
        <input
          ...
          onChange={(e) => {
            ...
            updateRange(e.target.value, rangeEnd)
          }}
          placeholder="Min"
          className="enhanced-logic-range-min"
        />
        <span className="enhanced-logic-range-separator">and</span>
        <input
          type="number"
          value={rangeEnd}
          onChange={(e) => {
            setRangeEnd(e.target.value)
            updateRange(rangeStart, e.target.value)
          }}
          placeholder="Max"
          className="enhanced-logic-range-max"
        />
      </div>
    )
  }

  if (["is one of", "is not one of"].includes(condition)) {
    return (
      <div className={`enhanced-logic-multi-select ${className}`}>
        {options.map((option) => (
          <div key={option} className="enhanced-logic-option">
            <label className="enhanced-option-label">
              <input
                type="checkbox"
                ...
                onChange={(e) => {
                  if (e.target.checked) {
                    onChange(Array.isArray(value)
                      ? [...(value as string[]), option] as string[]
                      : [option]
                    )
                  } else {
                    onChange(Array.isArray(value)
  ...
  if (questionType === "Yes/No" && ["is", "is not"].includes(condition)) {
    return (
      <div className={`enhanced-logic-yes-no-select ${className}`}>
        <div
          className="enhanced-yes-no-option"
          onClick={() => onChange("Yes")}
        >
          <span className={value === "Yes" ? "selected" : ""}>Yes</span>
        </div>
        <div
          className="enhanced-yes-no-option"
          onClick={() => onChange("No")}
        >
          <span className={value === "No" ? "selected" : ""}>No</span>
        </div>
        <div
          className="enhanced-yes-no-option"
          onClick={() => onChange("N/A")}
        >
          <span className={value === "N/A" ? "selected" : ""}>N/A</span>
        </div>
      </div>
    )
  }

  if (questionType === "Number" || ["less than", "less than or equal to", "equal to", "not equal to", "greater than or equal to", "greater than"].includes(condition)) {
    return (
      <input
        type="number"
...
const EnhancedLogicTriggerSelector: React.FC<{
  ...
  const triggerLabels: Record<TriggerAction, string> = {
  ...
  return (
    <div className={`enhanced-logic-trigger-selector ${className}`} ref={dropdownRef}>
      <div className="selected-trigger" onClick={() => setIsOpen(!isOpen)}>
        <span className="trigger-text">{selectedTrigger ? triggerLabels[selectedTrigger] : "Select trigger"}</span>
        <ChevronDown className={`dropdown-arrow ${isOpen ? "rotate" : ""}`} />
      </div>
      {isOpen && (
        <div className="triggers-dropdown">
          {triggers.map((trigger) => (
            <div
              key={trigger}
              className={`trigger-option ${trigger === selectedTrigger ? "selected" : ""}`}
              onClick={() => {
      ...
      <div className="enhanced-logic-rule-content">
        ...
        <div className="enhanced-logic-trigger-row">
          ...
          <EnhancedLogicTriggerSelector
            ...
            onTriggerSelect={(trigger) => setLocalRule({ ...localRule, trigger })}
          />
        </div>
      </div>
...
const Garment_Template: React.FC = () => {
      ...
      >
        ...
        <div className="question-footer">
          ...
          <EnhancedAddLogicButton
            hasRules={question.logicRules !== undefined && question.logicRules.length > 0}
            onClick={() => setActiveLogicQuestionId(question.id)}
          />
        ...
        {activeLogicQuestionId === question.id && (
          <div className="logic-rules-overlay">
            <EnhancedLogicRulesContainer
              questionType={question.responseType}
              rules={question.logicRules || []}
              options={question.options}
              onRulesChange={(rules) => handleLogicRulesChange(question.id, rules)}
              onClose={() => setActiveLogicQuestionId(null)}
              className="logic-rules-container"
            />
          </div>
        )}
      </div>
  ...
  const handleLogicRulesChange = (questionId: string, rules: LogicRule[]) => {
    const sectionId = template.sections.find((section) => {
      if (section.type === "standard") {
        return (section.content as StandardSectionContent).questions.some((q) => q.id === questionId)
      }
      return false
    })?.id

    if (sectionId) {
      updateQuestion(sectionId, questionId, { logicRules: rules })
    }
  }
