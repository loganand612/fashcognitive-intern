...
class PDFExportService {
  ...
  private addHeader(): void {
    ...
    if (this.template.logo) {
      try {
        ...
        if (this.template.logo.startsWith('data:image/')) {
          this.doc.addImage(this.template.logo, "JPEG", this.margin, this.currentY, 25, 25)
          logoAdded = true;
    ...
    this.doc.text("GARMENT INSPECTION REPORT", this.pageWidth / 2, this.currentY + 10, { align: "center" })

    // Add non-negotiable text
    this.doc.setFontSize(10)
    this.doc.setFont("helvetica", "normal")
    this.doc.text("THIS DOCUMENT IS NON-NEGOTIABLE", this.pageWidth / 2, this.currentY + 18, { align: "center" })

    // Draw border around header
    this.doc.rect(this.margin, this.currentY, this.contentWidth, 25)

    // Draw vertical line to separate logo from title
    if (logoAdded) {
      this.doc.line(this.margin + 30, this.currentY, this.margin + 30, this.currentY + 25)
    }

    this.currentY += 30 // Move down after header
  ...
  private addReportInfo(): void {
    ...
    const reportNo = this.findQuestionValue(standardContent.questions, "Report No")
    const factoryName = this.findQuestionValue(standardContent.questions, "Factory Name")
    const styleNo = this.findQuestionValue(standardContent.questions, "Style No")
    ...
    const leftCol = [reportInfoData[0], reportInfoData[1]]
    const rightCol = [reportInfoData[2], reportInfoData[3]]

    // Add left column
    this.doc.autoTable({
      startY: this.currentY,
      margin: { left: this.margin },
      tableWidth: this.contentWidth / 2,
      body: leftCol,
      theme: "grid",
    ...
    this.doc.autoTable({
      startY: this.currentY,
      margin: { left: this.margin + this.contentWidth / 2 },
      tableWidth: this.contentWidth / 2,
      body: rightCol,
      theme: "grid",
      styles: { fontSize: 10, cellPadding: 3 },
    ...
    this.currentY += 20 // Move down after report info
  }
  ...
  private addInspectionDetails(): void {
    ...
    this.doc.setFont("helvetica", "bold")
    this.doc.text("Inspection Details", this.margin, this.currentY)
    this.currentY += 8
    ...
    standardContent.questions.forEach((question: Question) => {
      if (!["Site conducted", "Prepared by", "Location", "Report No", "Factory Name", "Style No", "Inspection date"].includes(question.text) && question.value) {
        inspectionDetails.push([question.text, question.value.toString()])
      }
    })
