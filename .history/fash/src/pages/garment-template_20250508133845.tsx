I'll update your existing files to include the enhanced PDF export functionality while maintaining your current UI. Here's the updated `garment-template.tsx` file with the new PDF export features integrated:
...
class PDFExportService {
  ...
  private addReportInfo(): void {
    ...
    const reportNo = this.findQuestionValue(titlePageSection.content.questions, "Report No")
    const factoryName = this.findQuestionValue(titlePageSection.content.questions, "Factory Name")
    const styleNo = this.findQuestionValue(titlePageSection.content.questions, "Style No")
    const inspectionDate = this.findQuestionValue(titlePageSection.content.questions, "Inspection date") || 
  ...
  private addInspectionDetails(): void {
    ...
    // Add site conducted
    const siteConducted = this.findQuestionValue(titlePageSection.content.questions, "Site conducted")
    if (siteConducted) {
      inspectionDetails.push(["Site conducted", siteConducted])
    }

    // Add prepared by
    const preparedBy = this.findQuestionValue(titlePageSection.content.questions, "Prepared by")
    if (preparedBy) {
      inspectionDetails.push(["Prepared by", preparedBy])
    }

    // Add location
    const location = this.findQuestionValue(titlePageSection.content.questions, "Location")
    if (location) {
      inspectionDetails.push(["Location", location])
    }

    // Add other questions from the title page
    titlePageSection.content.questions.forEach(question => {
  ...
  private addDefectsSection(): void {
    ...
    if (this.reportData.defects && this.reportData.defects.length > 0) {
      ...
      // Create data rows
      const body = this.reportData.defects.map((defect, index) => {
        // Calculate total
