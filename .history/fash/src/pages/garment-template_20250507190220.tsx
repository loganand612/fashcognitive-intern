                ...
                <table className="garment-table-preview">
                  ...
                  <tbody>
                    {(() => {
                      const garmentSection = template.sections.find((s) => s.type === "garmentDetails");
                      if (!garmentSection || !isGarmentDetailsContent(garmentSection.content)) {
                        return null; // Or some fallback UI like <tr><td colSpan="100%">No garment details</td></tr>
                      }
                      // Now TypeScript knows garmentSection.content is GarmentDetailsContent
                      return garmentSection.content.colors.map((color: string) => (
                        <tr key={color}>
                          <td>{color}</td>
                          {(() => {
                            // This inner IIFE for sizes can reuse the 'garmentSection.content' from the outer scope
                            // or keep its own find if preferred, though reusing is more efficient.
                            // For clarity and using the already type-guarded content:
                            if (!isGarmentDetailsContent(garmentSection.content)) return null; // Should not happen if outer check passed
