...
interface SlideBase {
  ...
  exitButton?: boolean
  doneText?: string
  items?: string[] | { title: string; description: string; image?: string }[]
  leftProduct?: {
    name: string
    tagline: string
  }
...
export default function Training() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<"text" | "image" | "media">("text")
  ...
  useEffect(() => {
    if (selectedSlide?.type === "horizontal-series" && selectedSlide.items?.length) {
      const interval = setInterval(() => {
        setActiveSeriesIndex((prev) => (prev + 1) % selectedSlide.items!.length)
      }, 3000)
    ...
    switch (templateId) {
      ...
      case "horizontal-series":
        newSlide = {
          ...
          items: [
            { title: "Product X", description: "Brilliant. In every way." },
            { title: "Product Y", description: "Almost as good." },
            { title: "Product Z", description: "The newest addition." },
          ],
          doneText: "Continue",
      ...
      case "quote":
      case "scrolling-text":
        // These templates are not implemented yet
        console.warn(`Slide template "${templateId}" is not implemented`)
        return
      default:
        return
    }
  ...
  const handleUpdateListItem = (index: number, value: string): void => {
    ...
    const updatedItems = [...(selectedSlide.items as string[])]
  ...
  const handleAddListItem = (): void => {
    ...
    const updatedItems = [...(selectedSlide.items as string[]), "New item"]
    handleUpdateSlide("items", updatedItems)
  }
      ...
      <div className="editor-header">
        <div className="header-content">
          ...
          <div className="header-right">
            <div className="saved-status">Saved</div>
                    ...
                    >
                      {lesson.slides.map((slide, index) => (
                        ...
                        >
                          ...
                          {slide.type === "end" ? <Lock className="slide-icon" /> : <FileText className="slide-icon" />}
                ...
                <div className="slide-body">
                  {selectedSlide?.type === "title" && (
                    <div className="title-slide">
                      ...
                      {selectedSlide.subtitle && <p className="slide-subtitle">{selectedSlide.subtitle}</p>}
                  ...
                  {selectedSlide?.type === "bulleted-list" && (
                    <div className="bulleted-list-slide">
                      ...
                      <ul className="bullet-list">
                        {(selectedSlide.items as string[])?.map((item, index) => (
                    ...
                    <div className="horizontal-series-slide">
                      ...
                      <div className="series-container">
                        <div className="series-content">
                          ...
                          >
                            {(selectedSlide.items as { title: string; description: string; image?: string }[])?.map(
                              (item, index) => (
                                <div key={index} className="series-item">
                                  <div className="product-image blue"></div>
                                  <div className="product-info">
                                    <div className="product-name">{item.title}</div>
                                    <div className="product-tagline">{item.description}</div>
                                  </div>
                                </div>
                              )
                            )}
                        ...
                        <div className="series-dots">
                          {selectedSlide.items?.map((_, index) => (
                            <span
                              key={index}
            ...
            {selectedSlide?.type === "horizontal-series" && (
              <div className="series-editor">
                ...
                <div className="form-group">
                  ...
                  {(selectedSlide.items as { title: string; description: string; image?: string }[])?.map(
                    (item, index) => (
                      <div key={index} className="series-item-editor">
                        <div className="item-header">
                          <span>Item {index + 1}</span>
                          <button className="item-control-button delete">
                            <X />
                          </button>
                        </div>
                        <input
                          ...
                          value={item.title}
                          onChange={(e) => {
                            const updatedItems = [...(selectedSlide.items as { title: string; description: string; image?: string }[])]
                            updatedItems[index] = {
                              ...updatedItems[index],
