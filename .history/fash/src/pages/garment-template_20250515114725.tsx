...
import { ChevronDown, ChevronUp, Edit, Plus, Calendar, User, MapPin, X, Check, ImageIcon, Trash2, Move, Clock, ArrowLeft, ArrowRight, CheckCircle, Settings, Ruler, Box, List, Shirt, FileText, Printer, Bell, MessageSquare, AlertTriangle, Upload, CornerDownRight, Equal, Hash, CircleEqual, CircleSlash, CircleDot, ChevronsRight, ChevronsLeft, ListFilter } from 'lucide-react'
    ...
    try {
      switch (rule.condition) {
        ...
        case "between":
          ...
          break
        case "is one of":
          conditionMet = Array.isArray(rule.value) && rule.value.includes(String(value))
          break
        case "is not one of":
          conditionMet = Array.isArray(rule.value) && !rule.value.includes(String(value))
          break
                    ...
                    (position) => {
                      ...
                      fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${position.coords.latitude}&lon=${position.coords.longitude}`)
                        ...
                        .catch(error => {
                          ...
                          updateQuestion(sectionId, coords);
                        });
                    },
                    (error) => {
...
// Add the following function to render trigger UI in the report page
                                      ...
                                      <div className="defect-images-preview">
                                        {(defectImages[index] || []).map((image, imageIndex) => (
                                          <div key={imageIndex} className="defect-image-item">
                                            <img src={image || "/placeholder.svg"} alt={`Defect ${index + 1} - Image ${imageIndex + 1}`} />
