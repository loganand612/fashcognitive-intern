...
import { ChevronDown, ChevronUp, Edit, Plus, Calendar, User, MapPin, X, Check, ImageIcon, Trash2, Move, Clock, ArrowLeft, ArrowRight, CheckCircle, Settings, Ruler, Box, List, Shirt, FileText } from 'lucide-react'
import "./garment-template.css"
import AccessManager from "./components/AccessManager"
import jsPDF from "jspdf"
// Import jspdf-autotable
import 'jspdf-autotable'
// Add the autoTable function to jsPDF instance
...
// @ts-ignore - unused for now but will be used in future updates
type LogicOperator =
  | "equals"
  | "notEquals"
  | "greaterThan"
...
class PDFExportService {
  ...
  private defectImages: { [defectIndex: number]: string[] }
  private currentY = 0
  private pageWidth = 210 // A4 width in mm
  private pageHeight = 297 // A4 height in mm
  private margin = 15
  private contentWidth: number = 210 - 30 // Page width minus margins

  constructor(template: Template, reportData: ReportData, defectImages: { [defectIndex: number]: string[] }) {
  ...
  public async generatePDF(): Promise<Blob> {
    try {
      ...
      try {
        ...
        console.error("Error adding report info:", error)
        // Continue with the rest of the PDF
      }
      ...
      try {
        ...
        console.error("Error adding inspection details:", error)
        // Continue with the rest of the PDF
      }

      try {
        // Add garment details section
        this.addGarmentDetails()
      } catch (error) {
        console.error("Error adding garment details:", error)
        // Continue with the rest of the PDF
      }
      ...
      try {
        ...
        console.error("Error adding carton info:", error)
        // Continue with the rest of the PDF
      }

      try {
        // Add defects section
        this.addDefectsSection()
      } catch (error) {
        console.error("Error adding defects section:", error)
        // Continue with the rest of the PDF
      }

      try {
        // Add photographs section
        this.addPhotographsSection()
      } catch (error) {
        console.error("Error adding photographs section:", error)
        // Continue with the rest of the PDF
      }
      ...
      try {
        ...
        console.error("Error adding AQL results:", error)
        // Continue with the rest of the PDF
      }
  ...
  private addHeader(): void {
    ...
    // Add logo section (left side)
    const logoWidth = 30
    this.doc.setDrawColor(0)
    this.doc.setLineWidth(0.5)
    this.doc.rect(this.margin, this.currentY, logoWidth, 25)
    ...
    if (this.template.logo) {
      try {
        ...
        if (this.template.logo.startsWith('data:image/')) {
          // Calculate logo position to center it in the logo box
          const logoSize = 20
          const logoX = this.margin + (logoWidth - logoSize) / 2
          const logoY = this.currentY + (25 - logoSize) / 2

          this.doc.addImage(this.template.logo, "JPEG", logoX, logoY, logoSize, logoSize)
          logoAdded = true;
          console.log("Logo added successfully")
        } else {
          console.error("Invalid logo format:", this.template.logo.substring(0, 30) + "...")
        }
      } catch (error) {
    ...
    // If no logo was added, add a placeholder text
    if (!logoAdded) {
      this.doc.setFontSize(12)
      this.doc.setFont("helvetica", "bold")
      this.doc.text("M.I.S", this.margin + logoWidth/2, this.currentY + 25/2, { align: "center" })
    }

    // Add title section (right side)
    const titleX = this.margin + logoWidth
    const titleWidth = this.contentWidth - logoWidth

    // Add title
    this.doc.setFontSize(16)
    this.doc.setFont("helvetica", "bold")
    this.doc.text("GARMENT INSPECTION REPORT", titleX + titleWidth/2, this.currentY + 8, { align: "center" })

    // Add non-negotiable text
    this.doc.setFontSize(9)
    this.doc.setFont("helvetica", "normal")
    this.doc.text("THIS DOCUMENT IS NON-NEGOTIABLE", titleX + titleWidth/2, this.currentY + 15, { align: "center" })

    // Draw horizontal line to separate title from report info
    this.doc.line(titleX, this.currentY + 18, titleX + titleWidth, this.currentY + 18)

    this.currentY += 25 // Move down after header
    console.log("Header added successfully")
  }

  private addReportInfo(): void {
    // Find the title page section
    const titlePageSection = this.template.sections.find(
      (section) => section.type === "standard" && section.title === "Title Page"
    )

    if (!titlePageSection || titlePageSection.type !== "standard") return

    // Type assertion to ensure we're working with StandardSectionContent
    const standardContent = titlePageSection.content as StandardSectionContent

    // Get report metadata from questions
    const reportNo = this.findQuestionValue(standardContent.questions, "Report No") || "1234-5678"
    const factoryName = this.findQuestionValue(standardContent.questions, "Factory Name") || "Canon/ Globe"
    const styleNo = this.findQuestionValue(standardContent.questions, "Style No") || "5241113400"
    const inspectionDate = this.findQuestionValue(standardContent.questions, "Inspection date") ||
                          new Date().toLocaleDateString()

    // Create a table for report information
    const reportInfoData = [
      [{ content: "Report No.", styles: { fontStyle: 'bold', fillColor: [245, 245, 245] } },
       { content: reportNo, styles: { halign: 'center' } }],
      [{ content: "Factory Name", styles: { fontStyle: 'bold', fillColor: [245, 245, 245] } },
       { content: factoryName, styles: { halign: 'center' } }],
      [{ content: "Style no.", styles: { fontStyle: 'bold', fillColor: [245, 245, 245] } },
       { content: styleNo, styles: { halign: 'center' } }],
      [{ content: "Inspection Date", styles: { fontStyle: 'bold', fillColor: [245, 245, 245] } },
       { content: inspectionDate, styles: { halign: 'center' } }],
    ]

    // Calculate the position - this should be in the header area
    const logoWidth = 30
    const titleX = this.margin + logoWidth
    const titleWidth = this.contentWidth - logoWidth

    // Add the table in the header area
    this.doc.autoTable({
      startY: this.currentY - 7, // Position it in the lower part of the header
      margin: { left: titleX },
      tableWidth: titleWidth,
      body: [
        [reportInfoData[0][0], reportInfoData[0][1], reportInfoData[2][0], reportInfoData[2][1]],
        [reportInfoData[1][0], reportInfoData[1][1], reportInfoData[3][0], reportInfoData[3][1]],
      ],
      theme: "grid",
      styles: { fontSize: 8, cellPadding: 2, lineWidth: 0.1 },
      columnStyles: {
        0: { cellWidth: titleWidth * 0.2 },
        1: { cellWidth: titleWidth * 0.3 },
        2: { cellWidth: titleWidth * 0.2 },
        3: { cellWidth: titleWidth * 0.3 },
      },
    })

    this.currentY += 5 // Small adjustment after report info
  }

  private findQuestionValue(questions: Question[], questionText: string): string | null {
    const question = questions.find(q => q.text === questionText)
    if (!question || !question.value) return null
    return question.value.toString()
  }

  private addInspectionDetails(): void {
    // Find the title page section
    const titlePageSection = this.template.sections.find(
      (section) => section.type === "standard" && section.title === "Title Page"
    )
    ...
    // Get AQL information
    const garmentSection = this.template.sections.find((s) => s.type === "garmentDetails")
    let aqlLevel = "2.5"
    let inspectionLevel = "II"
    let samplingPlan = "Single"
    let severity = "Normal"
    let status = "PASS"

    if (garmentSection && isGarmentDetailsContent(garmentSection.content)) {
      aqlLevel = this.reportData.aqlSettings.aqlLevel
      inspectionLevel = this.reportData.aqlSettings.inspectionLevel
      samplingPlan = this.reportData.aqlSettings.samplingPlan
      severity = this.reportData.aqlSettings.severity
      status = this.reportData.aqlSettings.status
    }

    // Create AQL result section
    this.doc.setFillColor(230, 240, 230) // Light green background
    this.doc.rect(this.margin, this.currentY, this.contentWidth, 25, 'F')

    this.doc.setFontSize(12)
    this.doc.setFont("helvetica", "bold")
    this.doc.setTextColor(0, 0, 0)
    this.doc.text("AQL Result", this.margin + 5, this.currentY + 7)

    // Add AQL details
    this.doc.setFontSize(9)
    this.doc.setFont("helvetica", "normal")

    const aqlX = this.margin + 5
    const levelY = this.currentY + 15

    this.doc.text(`AQL Level: ${aqlLevel}`, aqlX, levelY)
    this.doc.text(`Inspection Level: ${inspectionLevel}`, aqlX + 40, levelY)
    this.doc.text(`Sampling Plan: ${samplingPlan}`, aqlX + 90, levelY)
    this.doc.text(`Severity: ${severity}`, aqlX + 140, levelY)

    // Add status with background color
    const statusX = this.margin + this.contentWidth - 40
    const statusWidth = 35
    const statusY = this.currentY + 5
    const statusHeight = 15

    if (status === "PASS") {
      this.doc.setFillColor(200, 250, 200) // Green for PASS
    } else {
      this.doc.setFillColor(250, 200, 200) // Red for FAIL
    }

    this.doc.rect(statusX, statusY, statusWidth, statusHeight, 'F')
    this.doc.setFontSize(10)
    this.doc.setFont("helvetica", "bold")
    this.doc.text(`Status: ${status}`, statusX + statusWidth/2, statusY + statusHeight/2 + 3, { align: "center" })

    this.currentY += 30 // Move down after AQL section

    // Get inspection questions and answers
    const inspectionDetails = []

    // Add site conducted
    const siteConducted = this.findQuestionValue(standardContent.questions, "Site conducted")
    if (siteConducted) {
      inspectionDetails.push([{ content: "Site conducted", styles: { fontStyle: 'bold' } }, siteConducted])
    }

    // Add prepared by
    const preparedBy = this.findQuestionValue(standardContent.questions, "Prepared by")
    if (preparedBy) {
      inspectionDetails.push([{ content: "Prepared by", styles: { fontStyle: 'bold' } }, preparedBy])
    }

    // Add location
    const location = this.findQuestionValue(standardContent.questions, "Location")
    if (location) {
      inspectionDetails.push([{ content: "Location", styles: { fontStyle: 'bold' } }, location])
    }

    // Add other questions from the title page
    standardContent.questions.forEach((question: Question) => {
      if (!["Site conducted", "Prepared by", "Location", "Report No", "Factory Name", "Style No", "Inspection date"].includes(question.text) && question.value) {
        inspectionDetails.push([{ content: question.text, styles: { fontStyle: 'bold' } }, question.value.toString()])
      }
    })
    ...
    if (inspectionDetails.length > 0) {
      this.doc.autoTable({
        ...
        columnStyles: {
          0: { cellWidth: 50, fillColor: [248, 248, 248] }
        },
      })
  ...
  private addGarmentDetails(): void {
    console.log("Adding garment details section");

    // Add section header with background
    this.doc.setFillColor(240, 240, 245) // Light blue background
    this.doc.rect(this.margin, this.currentY, this.contentWidth, 8, 'F')

    this.doc.setFontSize(12)
    this.doc.setFont("helvetica", "bold")
    this.doc.setTextColor(0, 0, 0)
    this.doc.text("Garment Details", this.margin + 5, this.currentY + 5.5)

    this.currentY += 12

    // Check if we have quantity data
    if (Object.keys(this.reportData.quantities).length > 0) {
      console.log("Quantity data found:", this.reportData.quantities);

      // Get sizes and colors
      const colors = Object.keys(this.reportData.quantities)
      const sizes: string[] = []

      // Collect all unique sizes
      colors.forEach((color) => {
        Object.keys(this.reportData.quantities[color]).forEach((size) => {
      ...
      // First header row - Size labels
      const headerRow1 = [
        { content: 'COLOR', rowSpan: 2, styles: { fillColor: [245, 245, 250], fontStyle: 'bold', valign: 'middle' } }
      ]
      ...
      // Second header row - ORDER QTY and OFFERED QTY for each size
      const headerRow2 = []

      // Add ORDER QTY and OFFERED QTY for each size
      sizes.forEach(() => {
        headerRow2.push({
          content: 'ORDER QTY',
          styles: {
      ...
      // Add ORDER QTY and OFFERED QTY for TOTAL column
      headerRow2.push({
        content: 'ORDER QTY',
        styles: {
          fillColor: [240, 240, 250],
      ...
      const body = colors.map((color) => {
        ...
        // Add quantities for each size
        let totalOrderQty = 0
        let totalOfferedQty = 0

        sizes.forEach((size) => {
          const sizeData = this.reportData.quantities[color]?.[size] || { orderQty: "0", offeredQty: "0" }
          row.push(sizeData.orderQty || "0")
          row.push(sizeData.offeredQty || "0")

          totalOrderQty += Number.parseInt(sizeData.orderQty || "0", 10)
          totalOfferedQty += Number.parseInt(sizeData.offeredQty || "0", 10)
        })

        // Add totals
        row.push(totalOrderQty.toString())
        row.push(totalOfferedQty.toString())
      ...
      let grandTotalOfferedQty = 0

      sizes.forEach((size) => {
        let sizeOrderTotal = 0
        let sizeOfferedTotal = 0

        colors.forEach((color) => {
          const sizeData = this.reportData.quantities[color]?.[size] || { orderQty: "0", offeredQty: "0" }
          sizeOrderTotal += Number.parseInt(sizeData.orderQty || "0", 10)
          sizeOfferedTotal += Number.parseInt(sizeData.offeredQty || "0", 10)
        })

        totalsRow.push({
          content: sizeOrderTotal.toString(),
          styles: {
            fontStyle: 'bold',
            fillColor: [240, 240, 245]
          }
        ...
        totalsRow.push({
          content: sizeOfferedTotal.toString(),
          styles: {
            fontStyle: 'bold',
            fillColor: [240, 240, 245]
          }
        ...
        grandTotalOrderQty += sizeOrderTotal
        grandTotalOfferedQty += sizeOfferedTotal
      })

      totalsRow.push({
        content: grandTotalOrderQty.toString(),
        styles: {
          fontStyle: 'bold',
          fillColor: [240, 240, 245]
        }
      ...
      totalsRow.push({
        content: grandTotalOfferedQty.toString(),
        styles: {
          fontStyle: 'bold',
          fillColor: [240, 240, 245]
        }
      ...
      try {
        ...
        this.doc.autoTable({
          ...
          didDrawCell: (data: any) => {
            ...
            if (data.section === 'head' || data.section === 'body') {
              // Add vertical borders between size pairs (after every second column)
              if (data.column.index > 0 && data.column.index % 2 === 0 && data.column.index < (sizes.length * 2)) {
                const x = data.cell.x;
                const y = data.cell.y;
        ...
        const finalY = (this.doc as any).lastAutoTable.finalY
        this.currentY = finalY + 10
        console.log("Garment details table added successfully");
      } catch (error) {
        console.error("Error adding garment details table:", error);
        // Fallback to simple text
        this.doc.setFontSize(10)
        this.doc.setFont("helvetica", "italic")
        this.doc.text("Error rendering garment quantity table", this.margin, this.currentY)
        this.currentY += 10
      }
    } else {
      ...
      this.doc.setFontSize(10)
      this.doc.setFont("helvetica", "italic")
      this.doc.text("No garment quantity data available", this.margin, this.currentY)
      this.currentY += 10
    }
  }

  private addCartonInfo(): void {
    // Add section header with background
    this.doc.setFillColor(240, 240, 245) // Light blue background
    this.doc.rect(this.margin, this.currentY, this.contentWidth, 8, 'F')
    ...
    this.doc.setFont("helvetica", "bold")
    this.doc.setTextColor(0, 0, 0)
    this.doc.text("Carton Information", this.margin + 5, this.currentY + 5.5)

    this.currentY += 12

    const cartonData = []

    // Add carton offered if available
    if (this.reportData.cartonOffered) {
      cartonData.push([
        { content: "No. of Carton Offered:", styles: { fontStyle: 'bold', fillColor: [248, 248, 248] } },
        this.reportData.cartonOffered
      ])
    }

    // Add carton to inspect
    cartonData.push([
      { content: "No. of Carton to Inspect:", styles: { fontStyle: 'bold', fillColor: [248, 248, 248] } },
      this.reportData.cartonToInspect
    ])

    // Add carton inspected if available
    if (this.reportData.cartonInspected) {
      cartonData.push([
        { content: "No. of Carton Inspected:", styles: { fontStyle: 'bold', fillColor: [248, 248, 248] } },
        this.reportData.cartonInspected
      ])
    }
  ...
  private addDefectsSection(): void {
    ...
    this.doc.setFont("helvetica", "bold")
    this.doc.setTextColor(0, 0, 0)
    this.doc.text("Defects and Logs", this.margin + 5, this.currentY + 5.5)

    this.currentY += 12

    if (this.reportData.defects && this.reportData.defects.length > 0) {
      // Create header row
      const header = ["Defect Type", "Remarks", "Critical", "Major", "Minor", "Total", "Actions"]

      // Create data rows
      const body = this.reportData.defects.map((defect) => {
        // Calculate total
        const critical = Number.parseInt(defect.critical?.toString() || "0", 10)
        const major = Number.parseInt(defect.major?.toString() || "0", 10)
        const minor = Number.parseInt(defect.minor?.toString() || "0", 10)
        ...
        return [
          ...
          minor.toString(),
          total.toString(),
          ""  // Empty column for actions in UI (not shown in PDF)
        ]
      })

      // Add totals row
      const totalCritical = this.reportData.defects.reduce(
        (sum, defect) => sum + Number.parseInt(defect.critical?.toString() || "0", 10),
        0,
      )
      const totalMajor = this.reportData.defects.reduce(
        (sum, defect) => sum + Number.parseInt(defect.major?.toString() || "0", 10),
        0,
      )
      const totalMinor = this.reportData.defects.reduce(
        (sum, defect) => sum + Number.parseInt(defect.minor?.toString() || "0", 10),
        0,
      )
      const grandTotal = totalCritical + totalMajor + totalMinor
      ...
      this.doc.autoTable({
        ...
        columnStyles: {
          0: { fontStyle: "bold", fillColor: [248, 248, 248] },
          2: { halign: "center" },
          3: { halign: "center" },
          4: { halign: "center" },
          5: { halign: "center", fontStyle: "bold" },
          6: { cellWidth: 0 } // Hide actions column in PDF
        },
        headStyles: {
        ...
        didDrawCell: (data: any) => {
          // Add color to critical cells based on value
          if (data.column.index === 2 && data.row.index < body.length - 1) {
            ...
            if (value > 0) {
              ...
              this.doc.text(data.cell.text[0], data.cell.x + data.cell.width / 2, data.cell.y + data.cell.height / 2 + 3, { align: 'center' });
            }
          }

          // Add color to major cells based on value
          if (data.column.index === 3 && data.row.index < body.length - 1) {
            const value = parseInt(data.cell.text[0]);
            if (value > 0) {
              this.doc.setFillColor(255, 230, 200);
              this.doc.rect(data.cell.x, data.cell.y, data.cell.width, data.cell.height, 'F');
              this.doc.setTextColor(0);
              this.doc.text(data.cell.text[0], data.cell.x + data.cell.width / 2, data.cell.y + data.cell.height / 2 + 3, { align: 'center' });
            }
      ...
      this.doc.setFontSize(10)
      this.doc.setFont("helvetica", "italic")
      this.doc.text("No defect data available", this.margin, this.currentY)
      this.currentY += 10
    }
  }

  private addPhotographsSection(): void {
    // Check if we have any defect images
    if (Object.keys(this.defectImages).length === 0) {
      console.log("No defect images to add to PDF")
      return
    }

    try {
      console.log("Adding photographs section to PDF")

      // Add a new page for photographs
      this.doc.addPage()
      this.currentY = this.margin
      ...
      this.doc.setFont("helvetica", "bold")
      this.doc.setTextColor(0, 0, 0)
      this.doc.text("Photographs", this.margin + 5, this.currentY + 5.5)

      this.currentY += 12

      // Group images by defect type
      const defectTypes: Record<string, { images: string[]; remarks: string }> = {}

      Object.entries(this.defectImages).forEach(([indexStr, images]) => {
        const index = Number.parseInt(indexStr, 10)
        const defect = this.reportData.defects[index]

        if (defect && defect.type) {
          if (!defectTypes[defect.type]) {
            defectTypes[defect.type] = { images: [], remarks: defect.remarks || "" }
          }

          // Only add valid images
          const validImages = images.filter(img => img && img.startsWith('data:'))
          defectTypes[defect.type].images.push(...validImages)
        }
      })

      // Check if we have any valid images after filtering
      const totalImages = Object.values(defectTypes).reduce((count, data) => count + data.images.length, 0)
      if (totalImages === 0) {
        console.log("No valid images found after filtering")
        return
      }

      console.log(`Adding ${totalImages} images to PDF`)

      // Add images by defect type
      Object.entries(defectTypes).forEach(([defectType, data], typeIndex) => {
        if (data.images.length === 0) return // Skip if no valid images for this defect type

        // Add defect type header with background
        this.doc.setFillColor(230, 230, 240) // Light blue background for defect type
        this.doc.rect(this.margin, this.currentY, this.contentWidth, 7, 'F')

        this.doc.setFontSize(11)
        this.doc.setFont("helvetica", "bold")
        this.doc.setTextColor(0, 0, 0)
        this.doc.text(defectType, this.margin + 5, this.currentY + 5)
        ...
        if (data.remarks) {
          this.doc.setFontSize(9)
          this.doc.setFont("helvetica", "italic")
          this.doc.text(`Remarks: ${data.remarks}`, this.margin + 5, this.currentY)
          this.currentY += 8
        }

        // Add images in a grid (3 per row for better layout)
        const imagesPerRow = 3
        const imageGap = 5
        const imageWidth = (this.contentWidth - (imageGap * (imagesPerRow - 1))) / imagesPerRow
        const imageHeight = imageWidth * 0.75 // 4:3 aspect ratio

        for (let i = 0; i < data.images.length; i += imagesPerRow) {
          // Check if we need to add a new page
          ...
          for (let j = 0; j < imagesPerRow; j++) {
            if (i + j < data.images.length) {
              const imageX = this.margin + (imageWidth + imageGap) * j

              try {
                // Create a placeholder with border for the image
                this.doc.setDrawColor(200, 200, 200)
                this.doc.setLineWidth(0.3)
                this.doc.rect(imageX, this.currentY, imageWidth, imageHeight)

                // Try to add the image
                this.doc.addImage(data.images[i + j], "JPEG", imageX, this.currentY, imageWidth, imageHeight)

                // Add image number label
                this.doc.setFillColor(240, 240, 240)
                this.doc.rect(imageX, this.currentY + imageHeight - 5, 15, 5, 'F')
                this.doc.setFontSize(7)
                this.doc.setFont("helvetica", "normal")
                this.doc.setTextColor(100, 100, 100)
                this.doc.text(`#${i + j + 1}`, imageX + 7.5, this.currentY + imageHeight - 1.5, { align: "center" })

                console.log(`Successfully added image ${i + j + 1} for defect type ${defectType}`)
              } catch (error) {
                console.error(`Error adding image ${i + j + 1} for defect type ${defectType}:`, error)

                // Add error text in the placeholder
                this.doc.setFillColor(245, 245, 245)
                this.doc.rect(imageX, this.currentY, imageWidth, imageHeight, 'F')
                this.doc.setFontSize(8)
                this.doc.setFont("helvetica", "italic")
                this.doc.setTextColor(150, 150, 150)
                this.doc.text("Image could not be loaded", imageX + imageWidth/2, this.currentY + imageHeight/2, {
                  align: "center"
                })
              }
            }
        ...
        // Add space between defect types
        if (typeIndex < Object.keys(defectTypes).length - 1) {
          this.currentY += 5
        }
      })

      console.log("Photographs section added successfully")
    } catch (error) {
      console.error("Error in addPhotographsSection:", error)
      // Continue with the PDF generation even if the photographs section fails
    }
  }

  private addAQLResults(): void {
    // We've already added the AQL results at the top of the report in the addInspectionDetails method
    // This method is kept for backward compatibility but doesn't need to do anything
    console.log("AQL Results already added in inspection details section");
    return;
  }

  private addFooter(): void {
    // @ts-ignore - jsPDF type definition issue
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      this.doc.setFont("helvetica", "normal")
      this.doc.setTextColor(100, 100, 100)
      this.doc.text(`Page ${i} of ${totalPages}`, this.pageWidth - this.margin, this.pageHeight - 8, {
        align: "right",
      })

      // Add generation date
      this.doc.text(`Generated on ${new Date().toLocaleDateString()}`, this.margin, this.pageHeight - 8)

      // Add company name or copyright
      this.doc.text("© Fashcognitive Inspection System", this.pageWidth / 2, this.pageHeight - 8, {
        align: "center",
      })
    }
  }
...
const Garment_Template: React.FC = () => {
  ...
  const exportAsPdf = async () => {
    try {
      ...
      // Ensure we have at least some sample data for testing
      const enhancedReportData = {
        ...reportData,
        quantities: Object.keys(reportData.quantities).length > 0
          ? reportData.quantities
          : { "Black": { "S": { orderQty: "100", offeredQty: "95" }, "M": { orderQty: "200", offeredQty: "190" } } },
        cartonOffered: reportData.cartonOffered || "50",
        cartonToInspect: reportData.cartonToInspect || "10",
        cartonInspected: reportData.cartonInspected || "10",
        defects: reportData.defects.length > 0
          ? reportData.defects
          : [{ type: "Stitching", remarks: "Loose threads", critical: "1", major: "2", minor: "3" }],
      }

      console.log("Enhanced report data:", enhancedReportData);

      // Process images to ensure they're valid
      const processedDefectImages: { [defectIndex: number]: string[] } = {}

      // Only process images if there are any
      if (Object.keys(defectImages).length > 0) {
        console.log("Processing defect images...")
        for (const [indexStr, images] of Object.entries(defectImages)) {
          const index = Number.parseInt(indexStr, 10)
          processedDefectImages[index] = []

          for (const image of images) {
            try {
              // Ensure image is valid by creating a temporary image element
              const img = new Image()
              img.src = image
              ...
              // If we get here, the image is valid (or the timeout expired)
              processedDefectImages[index].push(image)
            } catch (imgError) {
      ...
      console.log("Creating PDF export service...")
      // Create a new PDF export service with processed images
      const pdfExportService = new PDFExportService(template, enhancedReportData, processedDefectImages)
      ...
      link.href = url
      link.download = `${template.title.replace(/\s+/g, "_")}_${new Date().toISOString().split("T")[0]}.pdf`
      ...
      alert("Failed to export PDF. Please try again. Error: " + (error instanceof Error ? error.message : "Unknown error"))
    } finally {
      ...
      <div className="report-preview-container">
        <div className="report-actions">
          ...
          <div className="report-action-buttons">
            <button className="export-pdf-button" onClick={exportAsPdf} disabled={isExporting}>
              <FileText size={18} />
