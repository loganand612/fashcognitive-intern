    ...
    const handleEnd = useCallback(() => {
      if (sigPadRef.current) {
        if (!sigPadRef.current.isEmpty()) {
          ...
          updateQuestion(sectionId, question.id, { value: null }); // Handle empty signature
        }
      }
    }, [sectionId, question.id, updateQuestion]); // Added updateQuestion to dependencies
    ...
    const handleEnd = useCallback(() => {
      if (sigPadRef.current) {
        if (!sigPadRef.current.isEmpty()) {
          ...
          updateQuestionAnswer(question.id, null); // Handle empty signature
        }
      }
    }, [question.id, updateQuestionAnswer]); // Added updateQuestionAnswer to dependencies
