...
// Moved useRef inside the component
export default function CreateTraining() {
  const slideLibraryRef: RefObject<HTMLDivElement> = useRef(null);

  // Fixed type errors for items and slides
  interface Slide {
    id: number;
    type: string;
    title: string;
    subtitle?: string;
    buttonText?: string;
    items?: Array<{ title: string; content?: string; description?: string }>;
  ...
  // Updated slide templates to match the correct types
  const SLIDE_TYPES: Record<string, Slide[]> = {
    TEXT: [
      ...
      },
      {
        ...
        before: { content: "Before content", label: "Before" },
        after: { content: "After content", label: "After" },
        buttonText: "Next",
      },
    ],
    IMAGE: [],
  ...
  const updateBulletedListItem = (index: number, value: string): void => {
    const updatedLessons = [...lessons]
    if (updatedLessons[currentLessonIndex].slides[currentSlideIndex].items) {
      updatedLessons[currentLessonIndex].slides[currentSlideIndex].items![index].title = value
      setLessons(updatedLessons)
    }
  }
  ...
  const addBulletedListItem = (): void => {
    const updatedLessons = [...lessons]
    if (!updatedLessons[currentLessonIndex].slides[currentSlideIndex].items) {
      updatedLessons[currentLessonIndex].slides[currentSlideIndex].items = []
    }
    updatedLessons[currentLessonIndex].slides[currentSlideIndex].items!.push({ title: "New item" })
    setLessons(updatedLessons)
  }
