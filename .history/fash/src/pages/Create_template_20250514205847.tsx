...
const shouldShowTrigger = (question: Question, triggerType: TriggerAction): boolean => {
  ...
  // Special case for require_evidence trigger - show it regardless of question value
  if (triggerType === "require_evidence") {
    // Check if there's an active rule for require_evidence
    return question.logicRules.some(rule =>
      rule.trigger === "require_evidence" && !(rule as any).tempDisabled
    );
  }

  // For other triggers, we need a question value
...
const Report: React.FC<{ template: Template }> = ({ template }) => {
            ...
            {response.responseType === "Evidence" && (
              <div className="report-logic-evidence">
                <div className="report-logic-response-header">
                  ...
                  <span className="report-logic-timestamp">{new Date().toLocaleDateString()}</span>
                </div>
                {response.responseEvidence && (
                  <>
                    <div
                      className="report-logic-evidence-image"
                      onClick={() => handleImageClick(response.responseEvidence!)}
                    >
                      <img
                      ...
                      />
                      <div className="report-logic-evidence-zoom-hint">
                        <Search size={16} /> Click to zoom
                      </div>
                    </div>
                    <div className="report-logic-evidence-caption">
                      Evidence provided for {question.text}
                    </div>
                  </>
                )}
              </div>
  ...
  return (
    <div className="report-container">
      ...
      {zoomedImage && (
        <div className="report-logic-evidence-zoom-overlay" onClick={closeZoomedImage}>
          <div className="report-logic-evidence-zoom-content" onClick={(e) => e.stopPropagation()}>
            <img src={zoomedImage} alt="Zoomed evidence" />
            <button className="report-logic-evidence-zoom-close" onClick={closeZoomedImage}>
              <X size={24} />
            </button>
          </div>
        </div>
...
const CreateTemplate: React.FC = () => {
  ...
  const renderTriggerUI = (question: Question, activeSection: Section) => {
    // Check for require_evidence trigger
                ...
                if (e.target.files && e.target.files[0]) {
                  ...
                  reader.onload = (event) => {
                    if (event.target?.result) {
                      // Get the rule that triggered this evidence requirement
                      const rule = question.logicRules?.find(r => r.trigger === "require_evidence");
                      ...
                      if (rule) {
                        ...
                        const newResponse = {
                          ...
                          responseType: "Evidence",
                          responseValue: "Evidence provided",
                          responseText: "Evidence for " + question.text,
