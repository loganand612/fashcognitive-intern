    ...
    const titlePageSection: AppSection = {
      ...
      content: {
        ...
        questions: [
          ...
          // Removed "Inspection date" question as per user request
          {
            id: generateId(),
            text: "Prepared by",
            responseType: "Person",
...
interface AnnotationResponseFieldProps {
  sectionId: string;
  question: Question;
  updateQuestion: (sectionId: string, questionId: string, updates: Partial<Question>) => void;
}

const AnnotationResponseField: React.FC<AnnotationResponseFieldProps> = ({ sectionId, question, updateQuestion }) => {
  const sigPadRef = useRef<SignaturePad | null>(null);

  const handleEnd = useCallback(() => {
    if (sigPadRef.current) {
      if (!sigPadRef.current.isEmpty()) {
        const dataUrl = sigPadRef.current.getTrimmedCanvas().toDataURL();
        updateQuestion(sectionId, question.id, { value: dataUrl });
      } else {
        updateQuestion(sectionId, question.id, { value: null });
      }
    }
  }, [sectionId, question.id, updateQuestion]);
...
const renderQuestionResponse = (question: Question, sectionId: string, updateQuestion: (sectionId: string, questionId: string, updates: Partial<Question>) => void) => {
  // Removed useRef and useCallback from here
        ...
        <div className="response-field media-field">
          <input
            ...
            onChange={(e) => {
              ...
              if (!files) return;
              // Handle file upload logic here, e.g., convert to base64 or upload to server
              // For demo, just log file names
              console.log("Uploaded media files:", Array.from(files).map(f => f.name));
            }}
...
const Garment_Template: React.FC = () => {
  ...
  const renderReportQuestionResponse = (question: Question, reportData: ReportData, updateQuestionAnswer: (questionId: string, value: string | string[] | boolean | number | null) => void) => {
    const value =
          ...
          <div className="report-response-field">
            <input
              ...
              onChange={(e) => {
                const files = e.target.files;
                if (!files) return;
                console.log("Uploaded media files in report:", Array.from(files).map(f => f.name));
              }}
        ...
        return (
          <div className="report-response-field">
            <SignaturePad
              width={300}
              height={150}
              onEnd={(dataUrl: string) => {
                updateQuestionAnswer(question.id, dataUrl);
              }}
            />
          </div>
        )
