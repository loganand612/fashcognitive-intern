...
const Report: React.FC<{ template: Template }> = ({ template }) => {
  ...
  const generatePDF = async () => {
    ...
    const scoreText = `Score: ${answeredQuestions}/${totalQuestions} (${((answeredQuestions / totalQuestions) * 100).toFixed(1)}%)`
    ...
    if (template.logo) {
      try {
        const resizedLogo = await resizeImage(template.logo)
        doc.addImage(resizedLogo, "JPEG", margin, 5, 40, 20)
      } catch (error) {
    ...
    doc.text(template.title, pageWidth / 2, 15, { align: "center" })
    doc.setFontSize(12)
    doc.text(scoreText, pageWidth / 2, 25, { align: "center" })
    yOffset += 30

    doc.setFontSize(12)
    doc.setTextColor(...textColor)
    const descriptionLines = doc.splitTextToSize(template.description, pageWidth - 2 * margin)
    doc.text(descriptionLines, margin, yOffset + 5)
    yOffset += 10 + descriptionLines.length * 5

    // Use Array.from to avoid TS2802
    for (const [sectionIndex, section] of Array.from(template.sections.entries())) {
      checkPageBreak(40)
      doc.setFontSize(14)
      doc.setTextColor(...primaryColor)
      doc.text(`${sectionIndex + 1}. ${section.title}`, margin + 5, yOffset + 7)
      yOffset += 15
      ...
      if (section.description) {
        ...
        yOffset += descLines.length * 5 + 5
      }

      const tableData = section.questions
        .filter((q: Question) => q.value !== null && q.value !== undefined)
        .map((q: Question, qIndex: number) => {
          const triggeredLogic = shouldShowTrigger(q, "require_action")
            ? "Action Required"
            : shouldShowTrigger(q, "require_evidence")
              ? "Evidence Required"
              : shouldShowTrigger(q, "notify")
                ? "Notification Sent"
                : shouldShowTrigger(q, "ask_questions")
                  ? "Follow-up Questions"
                  : shouldShowTrigger(q, "display_message")
                    ? "Message Displayed"
                    : ""

          let status = q.flagged ? "⚑ Flagged" : ""

          // Add logic trigger indicators to the status
          if (triggeredLogic) {
            status += status ? `, ${triggeredLogic}` : triggeredLogic
          }

          return [`${sectionIndex + 1}.${qIndex + 1} ${q.text}`, String(q.value), status]
        })
      ...
      if (tableData.length > 0) {
        ...
        doc.setTextColor(255, 255, 255)
        doc.text("Question", margin + 2, yOffset + 5)
        doc.text("Answer", margin + columnWidths[0] + 2, yOffset + 5)
        doc.text("Status", margin + columnWidths[0] + columnWidths[1] + 2, yOffset + 5)
        yOffset += rowHeight

        doc.setTextColor(...textColor)
        for (const row of tableData) {
          checkPageBreak(rowHeight + 5)
          const questionLines = doc.splitTextToSize(row[0], columnWidths[0] - 4)
          const answerLines = doc.splitTextToSize(row[1], columnWidths[1] - 4)
          const statusLines = doc.splitTextToSize(row[2], columnWidths[2] - 4)
          const maxLines = Math.max(questionLines.length, answerLines.length, statusLines.length)
          const cellHeight = rowHeight * maxLines

          doc.text(questionLines, margin + 2, yOffset + 5)
          doc.text(answerLines, margin + columnWidths[0] + 2, yOffset + 5)
          doc.text(statusLines, margin + columnWidths[0] + columnWidths[1] + 2, yOffset + 5)
          yOffset += cellHeight

          // If there's evidence, add it to the PDF
          const question = section.questions.find(
            (q) => q.text === row[0].substring(row[0].indexOf(" ") + 1) && q.conditionalProof,
          )
          ...
          if (question?.conditionalProof) {
            ...
            try {
              doc.setFontSize(8)
              doc.text("Evidence:", margin + 2, yOffset + 5)
              yOffset += 10
              doc.addImage(question.conditionalProof, "JPEG", margin + 2, yOffset, 60, 30)
              yOffset += 35
            } catch (error) {
              console.error("Error adding evidence to PDF:", error)
            }
          }
        }
        yOffset += 10
    ...
    for (let page = 1; page <= totalPages; page++) {
      ...
      doc.text(`Page ${page} of ${totalPages} | ${scoreText}`, pageWidth - margin, pageHeight - margin, {
        align: "right",
      })
    }
...
const CreateTemplate: React.FC = () => {
  ...
  const renderQuestionResponse = (question: Question, sectionId: string) => {
    switch (question.responseType) {
      ...
      case "Inspection location":
        return (
          <div className="response-field dropdown-field">
      ...
      default:
        return <div className="response-field">Placeholder</div>
    }
  }
  ...
  return (
    <div className="template-builder">
      ...
      <div className="builder-content">
        ...
        {activeTab === 3 && <div className="access-tab">Access Tab (To be implemented)</div>}
