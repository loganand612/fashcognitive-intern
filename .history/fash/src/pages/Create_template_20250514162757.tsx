...
const CreateTemplate: React.FC = () => {
  ...
  const renderTriggerUI = (question: Question, activeSection: Section) => {
    ...
    if (shouldShowTrigger(question, "ask_questions")) {
      ...
      // Function to remove the trigger rule
      const removeTriggerRule = () => {
        if (!rule) return;

        // Find and remove the rule that has the ask_questions trigger
        const updatedRules = question.logicRules?.filter(r => r.id !== rule.id) || [];
