...
const getInitialTemplate = (): Template => {
  const titlePageSection: Section = {
    id: generateId(),
    title: "Title Page",
    description: "The Title Page is the first page of your inspection report. You can customize it below.",
    questions: [
      {
        id: generateId(),
        text: "Site conducted",
...
const shouldShowTrigger = (question: Question, triggerType: TriggerAction): boolean => {
  ...
  const isStringArray = (val: unknown): val is string[] =>
    Array.isArray(val) && val.every((v: unknown) => typeof v === "string")
...
const EnhancedLogicValueInput: React.FC<{
  ...
  const handleOptionToggle = (option: string) => {
    ...
    onChange(
      currentValues.includes(option) ? currentValues.filter((v: string) => v !== option) : [...currentValues, option],
    )
  }
...
const Report: React.FC<{ template: Template }> = ({ template }): React.ReactElement => {
  ...
  const generatePDF = async () => {
    ...
    `;
      ...
      if (template.logo) {
        try {
          doc.addImage(template.logo, 'AUTO', margin, yOffset, 20, 20);
          yOffset += 25;
        } catch (error) {
      ...
      doc.text(template.title, margin, yOffset);

      doc.setFontSize(12);
      doc.setTextColor(100, 100, 100);
      doc.text(`${new Date().toLocaleDateString()} / ${preparedBy}`, margin, yOffset + 8);
      ...
      doc.setFillColor(scorePercentage === 100 ? 76 : 244, scorePercentage === 100 ? 175 : 67, scorePercentage === 100 ? 80 : 54);
      doc.roundedRect(pageWidth - margin - statusWidth - 10, yOffset, statusWidth + 10, 8, 4, 4, 'F');

      doc.setTextColor(255, 255, 255);
      doc.setFontSize(10);
      doc.text(statusText, pageWidth - margin - 5, yOffset + 5, { align: 'right' });
      ...
      // Draw stat boxes background
      doc.setFillColor(248, 249, 250);

      // Flagged items box
      doc.roundedRect(margin, yOffset, statBoxWidth, 40, 3, 3, 'F');
      doc.setDrawColor(230, 230, 230);
      doc.roundedRect(margin, yOffset, statBoxWidth, 40, 3, 3, 'S');

      // Actions box
      doc.roundedRect(margin + statBoxWidth + 10, yOffset, statBoxWidth, 40, 3, 3, 'F');
      doc.setDrawColor(230, 230, 230);
      doc.roundedRect(margin + statBoxWidth + 10, yOffset, statBoxWidth, 40, 3, 3, 'S');

      // Status box
      doc.roundedRect(margin + (statBoxWidth * 2) + 20, yOffset, statBoxWidth, 40, 3, 3, 'F');
      doc.setDrawColor(230, 230, 230);
      doc.roundedRect(margin + (statBoxWidth * 2) + 20, yOffset, statBoxWidth, 40, 3, 3, 'S');

      // Add stat labels
      doc.setFontSize(12);
      doc.setTextColor(100, 100, 100);
      doc.text("Flagged items", margin + 10, yOffset + 15);
      doc.text("Actions", margin + statBoxWidth + 20, yOffset + 15);
      doc.text("Status", margin + (statBoxWidth * 2) + 30, yOffset + 15);

      // Add stat values
      doc.setFontSize(16);
      doc.setTextColor(0, 0, 0);
      doc.text(flaggedItems.toString(), margin + 10, yOffset + 30);
      doc.text(actionItems.toString(), margin + statBoxWidth + 20, yOffset + 30);
      doc.text("In progress", margin + (statBoxWidth * 2) + 30, yOffset + 30);

      yOffset += 50;

      // Add site info section with grid layout
      doc.setFillColor(255, 255, 255);
      doc.setDrawColor(240, 240, 240);
      doc.roundedRect(margin, yOffset, pageWidth - (margin * 2), 80, 3, 3, 'FD');
      ...
      doc.setFontSize(12);
      doc.setTextColor(100, 100, 100);
      doc.text("Site conducted", margin + 10, yOffset + 15);

      doc.setFontSize(14);
      doc.setTextColor(0, 0, 0);
      doc.text(String(site), margin + 10, yOffset + 30);

      // Conducted on
      doc.setFontSize(12);
      doc.setTextColor(100, 100, 100);
      doc.text("Conducted on", margin + infoBoxWidth + 10, yOffset + 15);

      doc.setFontSize(14);
      doc.setTextColor(0, 0, 0);
      doc.text(String(conductedOn), margin + infoBoxWidth + 10, yOffset + 30);

      // Prepared by
      doc.setFontSize(12);
      doc.setTextColor(100, 100, 100);
      doc.text("Prepared by", margin + 10, yOffset + 50);

      doc.setFontSize(14);
      doc.setTextColor(0, 0, 0);
      doc.text(String(preparedBy), margin + 10, yOffset + 65);

      // Location
      doc.setFontSize(12);
      doc.setTextColor(100, 100, 100);
      doc.text("Location", margin + infoBoxWidth + 10, yOffset + 50);

      doc.setFontSize(14);
      doc.setTextColor(0, 0, 0);
      doc.text(String(location), margin + infoBoxWidth + 10, yOffset + 65);

      yOffset += 90;

      // Add tabs section
      doc.setFillColor(255, 255, 255);
      doc.roundedRect(margin, yOffset, pageWidth - (margin * 2), 15, 3, 3, 'F');

      // Draw tab buttons
      const tabWidth = (pageWidth - (margin * 2)) / 3;

      // Summary tab (active)
      doc.setFillColor(255, 255, 255);
      doc.rect(margin, yOffset, tabWidth, 15, 'F');

      // Flagged Items tab
      doc.setFillColor(240, 240, 240);
      doc.rect(margin + tabWidth, yOffset, tabWidth, 15, 'F');

      // Media tab
      doc.setFillColor(240, 240, 240);
      doc.rect(margin + (tabWidth * 2), yOffset, tabWidth, 15, 'F');

      // Tab labels
      doc.setFontSize(10);
      doc.setTextColor(0, 0, 0);
      doc.text("Summary", margin + (tabWidth / 2), yOffset + 10, { align: 'center' });

      doc.setTextColor(100, 100, 100);
      doc.text("Flagged Items", margin + tabWidth + (tabWidth / 2), yOffset + 10, { align: 'center' });
      doc.text("Media", margin + (tabWidth * 2) + (tabWidth / 2), yOffset + 10, { align: 'center' });
      ...
      doc.setFontSize(12);
      doc.setTextColor(100, 100, 100);
      doc.text(`This report provides an overview of the safety inspection conducted at ${site}.`, margin, yOffset);
      yOffset += 20;
      ...
      for (const section of template.sections) {
        // Add section title if it's not the title page
        ...
        for (const question of section.questions) {
          // Skip title page questions
          if (section.title === "Title Page" &&
            (question.responseType === "Site" ||
             question.responseType === "Inspection date" ||
          ...
          doc.setFontSize(11);
          doc.setTextColor(100, 100, 100);
          const answerText = question.responseType === "Media" || question.responseType === "Annotation"
            ? "Media uploaded"
            : String(question.value || "Not answered");
          doc.text(answerText, margin + 30, yOffset + 30);

          // Add flag if flagged
          if (question.flagged) {
            doc.setFillColor(244, 67, 54);
            doc.roundedRect(pageWidth - margin - 60, yOffset + 10, 50, 20, 3, 3, 'F');
            doc.setTextColor(255, 255, 255);
            doc.text("Flagged", pageWidth - margin - 35, yOffset + 22, { align: 'center' });
          ...
          if ((question.responseType === "Media" || question.responseType === "Annotation") && question.value) {
            try {
              ...
              doc.setFontSize(10);
              doc.setTextColor(100, 100, 100);
              doc.text(`Media for: ${question.text}`, margin, yOffset);
              yOffset += 5;
      ...
      if (flaggedItems > 0) {
        // Add a new page for flagged items
        doc.addPage();
        yOffset = margin + 20;
        ...
        doc.setFontSize(16);
        doc.setTextColor(0, 0, 0);
        doc.text("Flagged Items", margin, yOffset);
        yOffset += 10;

        // Add flagged count
        doc.setFontSize(12);
        doc.setTextColor(100, 100, 100);
        doc.text(`${flaggedItems} flagged, ${actionItems} action`, margin, yOffset);
        yOffset += 20;

        // Process each flagged question
        for (const section of template.sections) {
          for (const question of section.questions.filter(q => q.flagged)) {
            // Check if we need a new page
            if (yOffset > pageHeight - 60) {
              doc.addPage();
              yOffset = margin + 20;
            ...
            // Draw question box with red border
            doc.setFillColor(255, 255, 255);
            ...
            if ((question.responseType === "Media" || question.responseType === "Annotation") && question.value) {
              try {
                ...
                const imageValue = question.value as string;
                const processedImage = processedImages.has(imageValue)
                  ? processedImages.get(imageValue)
                  : imageValue;
      ...
      // Add media section if there are any media items
      ...
      if (mediaItems.length > 0) {
        ...
        doc.setFontSize(16);
        doc.setTextColor(0, 0, 0);
        doc.text("Media Summary", margin, yOffset);
        yOffset += 20;
        ...
        for (const media of mediaItems) {
          ...
          try {
            // Use cached processed image
            const processedImage = processedImages.has(media.thumbnail)
              ? processedImages.get(media.thumbnail)
              : media.thumbnail;

            // Add caption
            doc.setFontSize(10);
            doc.setTextColor(100, 100, 100);
            doc.text(`Media for: ${media.caption}`, margin, yOffset);
            yOffset += 5;
            ...
            doc.setTextColor(244, 67, 54);
            doc.text("Error loading media", margin, yOffset + 10);
            yOffset += 30;
          }
        }
      }
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        doc.text("Powered by SafetyCulture", pageWidth / 2, pageHeight - 10, { align: 'center' });
      }
...
const CreateTemplate: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    console.log("Saving template:", template) // Replace with actual save logic
  }
      ...
      <div className="mobile-preview">
        <div className="mobile-preview-header">
          ...
          <div className="mobile-save-actions">
            <button className="mobile-save-button" onClick={handleSave}>
              <Save size={20} />
            </button>
          </div>
        </div>
        ...
        <div className="mobile-section-header">
          <h2 className="mobile-section-title">{activeSection?.title}</h2>
          <p className="mobile-section-description">{activeSection?.description}</p>
        </div>
          ...
          {activeSection?.questions.map((question, index) => (
            <div key={question.id} className="mobile-question">
              <div className="mobile-question-header">
                <span className="mobile-question-number">{index + 1}.</span>
                <div className="mobile-question-text">
                  <span className="mobile-required">{question.required ? "*" : ""}</span>
                  {question.text}
                </div>
                {question.flagged && (
                  <div className="mobile-flagged-icon">
                    <Flag size={16} />
                  </div>
                )}
              </div>
