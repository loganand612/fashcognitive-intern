...
import { ChevronDown, ChevronUp, Edit, Plus, Calendar, User, MapPin, X, Check, ImageIcon, Trash2, Move, Clock, ArrowLeft, ArrowRight, CheckCircle, Settings, Ruler, Box, List, Shirt, FileText, Printer, Bell, AlertTriangle, MessageSquare, Equal, Hash, CircleEqual, CircleSlash, CircleDot, ChevronsRight, ChevronsLeft, ListFilter } from 'lucide-react'
...
const LOGIC_SUPPORTED_TYPES: ResponseType[] = [
  "Text",
  "Number",
  "Checkbox",
  "Yes/No",
...
const EnhancedLogicTriggerSelector: React.FC<{
  ...
  className?: string
}> = ({ selectedTrigger, onTriggerSelect, className = "" }) => {
  const [isOpen, setIsOpen] = useState(false)
  const dropdownRef = useRef<HTMLDivElement>(null)
...
const EnhancedLogicRuleBuilder: React.FC<{
  ...
  className?: string
}> = ({ questionType, rule, options = [], onRuleChange, onRuleDelete, className = "" }) => {
...
const EnhancedLogicRulesContainer: React.FC<{
  ...
  onClose: () => void
  className?: string
}> = ({ questionType, rules, options = [], onRulesChange, onClose, className = "" }) => {
  const addNewRule = () => {
...
// Helper function to generate a unique rule ID
        ...
        <div className="question-footer">
          ...
          {LOGIC_SUPPORTED_TYPES.includes(question.responseType) && (
            <EnhancedAddLogicButton
              hasRules={(question.logicRules?.length || 0) > 0}
              onClick={() => setShowLogicPanel(question.id)}
              className="question-logic-button"
            />
          )}
            ...
            <div className={`template-content ${showMobilePreview ? 'with-preview' : ''}`}>
              ...
              {showLogicPanel && (() => {
                const question = template.sections
                  ...
                  .flatMap((s) => (s.content as StandardSectionContent).questions)
                  .find((q) => q.id === showLogicPanel)

                if (!question) return null

                return (
                  <div className="logic-panel-overlay">
                    <EnhancedLogicRulesContainer
                      questionType={question.responseType}
                      rules={question.logicRules || []}
                      options={question.responseType === "Multiple choice" ? question.options || [] : []}
