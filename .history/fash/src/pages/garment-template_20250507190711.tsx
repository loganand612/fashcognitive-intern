            ...
            {template.sections.some((s) => s.type === "garmentDetails") ? (
              <div className="garment-grid-preview">
                ...
                <table className="garment-table-preview">
                  ...
                  <tbody>
                    {(() => {  // START OF NEW IIFE FOR COLORS
                      const garmentSection = template.sections.find((s) => s.type === "garmentDetails");
                      // Type guard check
                      if (!garmentSection || !isGarmentDetailsContent(garmentSection.content)) {
                        // If no garment section or content isn't garment details, render nothing or a placeholder
                        return null; 
                        // Alternatively, for better UI:
                        // return <tr><td colSpan={/* appropriate number of columns */}>Garment details not configured.</td></tr>;
                      }
                      // At this point, garmentSection.content is known to be GarmentDetailsContent
                      return garmentSection.content.colors.map((color: string) => ( // Explicitly type 'color'
                        <tr key={color}>
                          <td>{color}</td>
                          {(() => {
                            // Inner IIFE for sizes can reuse garmentSection.content
                            // No need to find 'garmentSection' again here.
                            // The outer check already confirmed garmentSection.content is GarmentDetailsContent.
                            return garmentSection.content.sizes.map((size: string) => ( // Explicitly type 'size'
                              <React.Fragment key={size}>
                                <td>
                                  <input
                                    type="number"
