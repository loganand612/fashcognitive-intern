        ...
        <nav className="dashboard-sidebar-nav">
          {menuItems.map((item, index) => (
            <a
              key={index}
              href={item.href}
              className={`dashboard-nav-link ${item.href === '/dashboard' ? 'active' : ''}`}
              style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}
            >
              <item.icon size={20} style={{ minWidth: '20px', minHeight: '20px' }} />
        ...
        <section className="dashboard-recent-activity dashboard-animate-on-scroll">
          ...
          <div className="dashboard-activity-list">
            {recentActivity.map((item, index) => (
              <div
                key={item.id}
                className="dashboard-activity-item dashboard-animate-on-scroll"
