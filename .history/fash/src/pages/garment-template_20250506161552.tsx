...
const Garment_Template: React.FC = (): React.ReactElement => {
  ...
  // Report functions
  const handleQuantityChange = (color: string, size: string, field: keyof QuantityData, value: string) => {
    setReportData((prev) => {
  ...
  const calculateRowTotal = (color: string, field: keyof QuantityData) => {
    const garmentContent = getGarmentDetailsContent(template.sections)
  ...
  const calculateColumnTotal = (size: string, field: keyof QuantityData) => {
    const garmentContent = getGarmentDetailsContent(template.sections)
  ...
  const calculateGrandTotal = (field: keyof QuantityData) => {
    const garmentContent = getGarmentDetailsContent(template.sections)
    const colors = garmentContent?.colors || []
    let total = 0
    colors.forEach((color) => {
  ...
  const calculateTotalDefects = (field: "major" | "minor") => {
  ...
  const renderQuestion = (question: Question, sectionId: string, index: number): React.ReactElement => {
  ...
  const renderGarmentDetailsSection = (section: AppSection): React.ReactElement | null => {
  ...
  const renderSection = (section: AppSection, index: number): JSX.Element => {
  ...
  const renderReportTab = (): JSX.Element => {
    return (
  ...
  const renderAccessTab = (): JSX.Element => {
    return (
