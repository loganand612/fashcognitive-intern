...
class PDFExportService {
  ...
  private addPhotographsSection(): void {
    ...
    try {
      ...
      this.doc.setFont("helvetica", "bold")
      this.doc.text("Photographs", this.margin, this.currentY)
      this.currentY += 10
      ...
      Object.entries(defectTypes).forEach(([defectType, data], typeIndex) => {
        ...
        // Add defect type header
        this.doc.setFontSize(12)
        this.doc.setFont("helvetica", "bold")
        this.doc.text(defectType, this.margin, this.currentY)
        this.currentY += 5
        ...
        if (data.remarks) {
          this.doc.setFontSize(10)
          this.doc.setFont("helvetica", "italic")
          this.doc.text(`Remarks: ${data.remarks}`, this.margin, this.currentY)
          this.currentY += 8
        }

        // Add images in a grid (2 per row)
        const imageWidth = (this.contentWidth - 10) / 2 // 10mm gap between images
        const imageHeight = imageWidth * 0.75 // 4:3 aspect ratio

        for (let i = 0; i < data.images.length; i += 2) {
          // Check if we need to add a new page
          if (this.currentY + imageHeight + 10 > this.pageHeight - this.margin) {
            this.doc.addPage()
            this.currentY = this.margin
          }
          ...
          try {
            ...
            this.doc.rect(this.margin, this.currentY, imageWidth, imageHeight)

            // Try to add the image
            this.doc.addImage(data.images[i], "JPEG", this.margin, this.currentY, imageWidth, imageHeight)
            console.log(`Successfully added image ${i+1} for defect type ${defectType}`)
          } catch (error) {
            console.error(`Error adding image ${i+1} for defect type ${defectType}:`, error)

            // Add error text in the placeholder
            this.doc.setFontSize(8)
            this.doc.setFont("helvetica", "italic")
            this.doc.text("Image could not be loaded", this.margin + imageWidth/2, this.currentY + imageHeight/2, {
              align: "center"
            })
          }
          ...
          if (i + 1 < data.images.length) {
            try {
              // Create a placeholder for the image
              this.doc.rect(this.margin + imageWidth + 10, this.currentY, imageWidth, imageHeight)

              // Try to add the image
              this.doc.addImage(
                data.images[i + 1],
                "JPEG",
                this.margin + imageWidth + 10,
                this.currentY,
                imageWidth,
                imageHeight,
              )
              console.log(`Successfully added image ${i+2} for defect type ${defectType}`)
            } catch (error) {
              console.error(`Error adding image ${i+2} for defect type ${defectType}:`, error)

              // Add error text in the placeholder
              this.doc.setFontSize(8)
              this.doc.setFont("helvetica", "italic")
              this.doc.text("Image could not be loaded", this.margin + imageWidth + 10 + imageWidth/2, this.currentY + imageHeight/2, {
                align: "center"
              })
            }
          }
  ...
  private addAQLResults(): void {
    ...
    this.doc.setFontSize(14)
    this.doc.setFont("helvetica", "bold")
    this.doc.text("AQL Result", this.margin, this.currentY)
    this.currentY += 8

    // Create AQL info table
    const aqlData = [
      ["AQL Level:", this.reportData.aqlSettings.aqlLevel],
      ["Inspection Level:", this.reportData.aqlSettings.inspectionLevel],
      ["Sampling Plan:", this.reportData.aqlSettings.samplingPlan],
      ["Severity:", this.reportData.aqlSettings.severity],
      ["Status:", this.reportData.aqlSettings.status],
    ]
    ...
    const statusBoxX = this.margin + (this.contentWidth - statusBoxWidth) / 2

    // Set color based on status
    const fillColor =
      this.reportData.aqlSettings.status === "PASS"
        ? [200, 250, 200] // Light green for PASS
        : [250, 200, 200] // Light red for FAIL

    // Draw status box
    this.doc.setFillColor(fillColor[0], fillColor[1], fillColor[2])
    this.doc.rect(statusBoxX, this.currentY, statusBoxWidth, statusBoxHeight, "F")
    ...
    this.doc.text(
      `Status: ${this.reportData.aqlSettings.status}`,
      this.pageWidth / 2,
      this.currentY + statusBoxHeight / 2 + 5,
      { align: "center" },
    )

    this.currentY += statusBoxHeight + 10
  }
  ...
  private addFooter(): void {
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      this.doc.setFontSize(8)
      this.doc.setFont("helvetica", "normal")
      this.doc.text(`Page ${i} of ${totalPages}`, this.pageWidth - this.margin, this.pageHeight - 10, {
        align: "right",
      })
...
const Garment_Template: React.FC = () => {
  ...
  const exportAsPdf = async () => {
    try {
      ...
      const pdfExportService = new PDFExportService(template, reportData, processedDefectImages)
