...
// Import React types directly to resolve JSX namespace issues
import type { ReactElement, DragEvent, FormEvent, ChangeEvent } from 'react'
import {
  ChevronDown,
  ChevronUp,
  Edit,
...
// Add enum for tab selection at the top with other types
enum TabType {
  BUILD = 0,
  REPORT = 1,
  ACCESS = 2,
...
const GarmentTemplateWrapper: React.FC<GarmentTemplateProps> = () => {
  const [template, setTemplate] = useState<Template>(getInitialTemplate())
  const [activeTab, setActiveTab] = useState<TabType>(TabType.BUILD)
  ...
  const handleDragOver = (type: "question" | "section", id: string, e: DragEvent<HTMLDivElement>): void => {
    e.preventDefault()
  ...
  const calculateRowTotal = (color: string, field: keyof QuantityData): number => {
    const garmentContent = getGarmentDetailsContent(template.sections);
    const sizes: string[] = garmentContent?.sizes || [];
    let total = 0;
  ...
  const calculateColumnTotal = (size: string, field: keyof QuantityData): number => {
    const garmentContent = getGarmentDetailsContent(template.sections);
    const colors: string[] = garmentContent?.colors || [];
    let total = 0;
  ...
  const renderGarmentDetailsSection = (section: AppSection): ReactElement | null => {
    const garmentContent = isGarmentDetailsContent(section.content) ? section.content : null;
    if (!garmentContent) return null;
  ...
  const renderSection = (section: AppSection, index: number): ReactElement => {
  ...
  const renderReportTab = (): ReactElement => {
    return (
  ...
  const renderAccessTab = (): ReactElement => {
    return (
  ...
  const getActiveTab = (tab: TabType): ReactElement => {
    switch (tab) {
      case TabType.BUILD:
        return (
      ...
      <div className="top-navigation">
        ...
        <div className="nav-center">
          <div className="nav-tabs">
            <button className={`nav-tab ${activeTab === TabType.BUILD ? "active" : ""}`} onClick={() => setActiveTab(TabType.BUILD)}>
              1. Build
            </button>
            <button className={`nav-tab ${activeTab === TabType.REPORT ? "active" : ""}`} onClick={() => setActiveTab(TabType.REPORT)}>
              2. Report
            </button>
            <button className={`nav-tab ${activeTab === TabType.ACCESS ? "active" : ""}`} onClick={() => setActiveTab(TabType.ACCESS)}>
              3. Access
            </button>
          </div>
        </div>
...
export default GarmentTemplateWrapper
