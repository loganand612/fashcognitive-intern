...
const Garment_Template: React.FC = () => {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (draggedItem.type === "section" && dropTarget.type === "section") {
      ...
      const [removedQuestion] = (newSections[draggedSectionIndex].content as StandardSectionContent).questions
        .splice(
          draggedQuestionIndex,
          1,
        )(newSections[dropSectionIndex].content as StandardSectionContent)
        .questions.splice(dropQuestionIndex, 0, removedQuestion)
  ...
  const renderReportTab = () => {
    const [reportData, setReportData] = useState(() => {
      const initialDefects =
        template.sections
          ...
          ?.content.defaultDefects.map((defect) => ({
            type: defect,
            remarks: "",
            major: 0,
            minor: 0,
      ...
      const initialAqlLevel =
        template.sections.find((s) => s.type === "garmentDetails")?.content.aqlSettings.aqlLevel || "2.5"
      const initialInspectionLevel =
        template.sections.find((s) => s.type === "garmentDetails")?.content.aqlSettings.inspectionLevel || "II"
      const initialSamplingPlan =
        template.sections.find((s) => s.type === "garmentDetails")?.content.aqlSettings.samplingPlan || "Single"
      const initialSeverity =
        template.sections.find((s) => s.type === "garmentDetails")?.content.aqlSettings.severity || "Normal"
    ...
    const handleQuantityChange = useCallback((color, size, field, value) => {
      setReportData((prev) => {
    ...
    const calculateTotalDefects = useCallback(
      (field) => {
        return reportData.defects.reduce((total, defect) => {
          const value = Number.parseInt(defect[field], 10)
    ...
    const updateAqlResult = useCallback((field, value) => {
      setReportData((prev) => ({
        ...prev,
                  ...
                  <tbody>
                    {reportData.defects.map((defect, index) => (
                      <tr key={index}>
                        ...
                        <td>{Number.parseInt(defect.major || 0, 10) + Number.parseInt(defect.minor || 0, 10)}</td>
                      </tr>
                    ))}
                    <tr className="total-row">
