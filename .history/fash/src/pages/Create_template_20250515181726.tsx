...
const EnhancedLogicConditionSelector: React.FC<{
  ...
  useEffect(() => {
    const conditions: LogicCondition[] = (() => {
      switch (questionType) {
        case "Text":
          return ["is", "is not", "contains", "does not contain", "starts with", "ends with", "matches (regex)"]
        case "Number":
        case "Slider":
          return [
            "less than",
...
const Report: React.FC<{ template: Template }> = ({ template }) => {
  ...
  const [reportData, setReportData] = useState<{
    subQuestionAnswers?: {
      [questionId: string]: {
        [ruleId: string]: string | number | boolean | string[];
      };
    };
  }>({});

  // Render active triggers based on question response
  const renderActiveTriggers = (question: Question, questionId: string) => {
    if (!question.logicRules || question.logicRules.length === 0) return null;

    // Check which rules should be triggered based on the question's value
    const activeTriggers = question.logicRules.filter(rule => {
      if (rule.tempDisabled) return false;

      // Check if the condition is met
      switch (rule.condition) {
        case "is":
          return question.value === rule.value;
        case "is not":
          return question.value !== rule.value;
        case "contains":
          return typeof question.value === 'string' && typeof rule.value === 'string' &&
            question.value.toLowerCase().includes(rule.value.toLowerCase());
        case "does not contain":
          return typeof question.value === 'string' && typeof rule.value === 'string' &&
            !question.value.toLowerCase().includes(rule.value.toLowerCase());
        case "greater than":
          return typeof question.value === 'number' && typeof rule.value === 'number' &&
            question.value > rule.value;
        case "less than":
          return typeof question.value === 'number' && typeof rule.value === 'number' &&
            question.value < rule.value;
        case "equal to":
        ...
        case "between":
          if (Array.isArray(rule.value) && rule.value.length === 2) {
            return typeof question.value === 'number' &&
              question.value >= rule.value[0] && question.value <= rule.value[1];
          }
          return false;
        case "not between":
          if (Array.isArray(rule.value) && rule.value.length === 2) {
          ...
          return false;
        case "is one of":
          return Array.isArray(rule.value) && rule.value.includes(question.value as string);
        case "is not one of":
          return Array.isArray(rule.value) && !rule.value.includes(question.value as string);
        default:
          return false;
    ...
    return (
      <div className="report-triggers">
        {activeTriggers.map((rule, index) => {
          switch (rule.trigger) {
            case "display_message":
              return (
                <div key={index} className="report-trigger report-message-trigger">
                  <div className="report-trigger-header">
                    <AlertCircle size={16} className="report-trigger-icon" />
                    <span className="report-trigger-text">Important message</span>
                  </div>
                  <div className="report-message">{rule.message}</div>
                </div>
              );
            ...
            case "require_evidence":
              return (
                <div key={index} className="report-trigger report-evidence-trigger">
                  <div className="report-trigger-header">
                    <Upload size={16} className="report-trigger-icon" />
                    ...
                    <div className="report-subquestion">
                      ...
                      {rule.subQuestion.responseType === "Text" && (
                        <input
                          ...
                          value={String(reportData.subQuestionAnswers?.[questionId]?.[rule.id] || "")}
                        />
                      )}
      ...
      <div className="report-logic-responses">
        ...
        {question.logicResponses.map((response, index) => (
          <div key={index} className="report-logic-response">
            ...
            {response.responseType !== "Evidence" && response.responseType !== "Action" && (
    ...
    <div className="report-container">
      ...
      <div className="report-card">
        ...
        <div className="report-site-info">
          <h3 className="report-section-title">Inspection Details</h3>
          <p className="report-section-description">Key information about this inspection session.</p>
                  ...
                  section.questions
                    ...
                    .map((question) => (
                      <div key={question.id} className={`report-question-item ${question.flagged ? "flagged" : ""}`}>
                        ...
                        {renderActiveTriggers(question, question.id)}
...
const CreateTemplate: React.FC = () => {
  ...
  // Auto-save functionality instead of manual save
  const autoSave = () => {
    updateTemplate({ lastSaved: new Date() })
    console.log("Auto-saving template:", template) // Replace with actual save logic
  }

  const handleBack = () => {
    if (window.confirm("Do you want to save before leaving?")) autoSave()
